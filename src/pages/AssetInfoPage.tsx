...
import { ScanMode, SCAN_MODE_LABELS, SCAN_MODE_COLORS, SCAN_MODE_TRANSITION } from '../types'
...
export default function AssetInfoPage({ assetInfo, mode, onActionComplete, onBack }: Props) {
  ...
  async function handleConfirm() {
    ...
    setLoading(true)
    try {
      const transitionId = SCAN_MODE_TRANSITION[mode]
      await transitionIssue(assetInfo.jiraIssueId, transitionId)
      onActionComplete(
        `✓ ${SCAN_MODE_LABELS[mode]} recorded for ${assetInfo.jiraIssueId}`,
        'success'
      )
    } catch (e) {
      const msg = e instanceof Error ? e.message : 'Transition failed.'
      onActionComplete(`${SCAN_MODE_LABELS[mode]} failed: ${msg}`, 'error')
    } finally {
      setLoading(false)
    }
  }
...
function InfoRow({ label, value, highlight = false }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex items-center justify-between py-2.5">
      ...
      <span className={`text-sm font-medium max-w-[60%] text-right ${highlight ? 'text-brand-400' : 'text-white'}`}>
        {value}
      </span>
    </div>
  )
